...
class BlockDiv extends Component{
    ...
    render() {
        ...
        return (<div ref={this.attach} className="block" style={{
            width: `${p2rem(b.width)}`,
...
class FrameEditor extends Component{
    ...
    static createBlock(start, end, type) {
        const block =  new Block({type, owner: null, x: 0, y: 0})
    ...
    findRightbox = () => {
        return findDOMNode(this).querySelector('.frame-rightbox')
    }
    ...
    render() {
        return (<section className="frame-editor">
            <section className="frame-content">
                ...
                <section className="frame-overlay" ref={this.attachOverlay} style={{
                    backgroundColor: this.state.showOverlay ? 'rgba(0,0,0,0.3)' : 'transparent'
                }}>
                    {this.state.currentBlock && <div className="block" style={{
                    ...
                    {this.state.blocks[BlockType.attack] && this.state.blocks[BlockType.attack].map((b, i) => {
                            return <BlockDiv block={b} key={i} update={this.updateBlock} />
                    })}
                   {this.state.blocks[BlockType.defense] && this.state.blocks[BlockType.defense].map((b, i) => {
                            return <BlockDiv block={b} key={i} update={this.updateBlock} />
                    })}
                </section>
            </section>
            <section className="frame-rightbox" ref={this.attachBox}>
                ...
                <footer>
                    <Button.Group>
                        <Button type="primary" onClick={this.save}>保存变更{this.props.frame.__modified__ && <sup data-show="true" className="ant-scroll-number ant-badge-dot"></sup>}</Button>
                        <Button>重置设置</Button>
                    </Button.Group>
                </footer>
            </section>
